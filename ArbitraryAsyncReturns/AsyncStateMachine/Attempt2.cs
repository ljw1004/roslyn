...
class Attempt2
{
    // GOAL: merely jiggle Attempt1 around so the sm is a field of the builder
    ...
    static IAsyncEnumerator<int> f(int p)
    {
        ...
        sm.builder = AsyncEnumeratorBuilder<int,fStateMachine>.Create();
        sm.state = -1;
        sm.builder.Start(ref sm);
        return sm.builder.Task;
    }

    class AsyncEnumeratorBuilder<T,TSM> : IAsyncEnumerator<T>
    {
